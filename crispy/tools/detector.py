...
def photonCounting(cimg,
    ...
    if ncr > 1:
        ...
        crtoy = crtoy/crtoy[ncrpxidx]*threshold*RN
...
#         crtoy[0:2] = 65536
        ...
        for i in range(ncr.astype(int)):
            crx = np.random.randint(0, xi-1)
            crx += xs-xi
            cry = np.random.randint(0, yi-1)
            cridx = cry*xs+crx
            if cridx > len(crimg)-1-xs: cridx = len(crimg)-1-xs
            # This is a deviation from Patrick's model as of June 2018
#             crimg[cridx:cridx+xs] = crtoy
            crimg[cridx:cridx+xs] += crtoy
#         crimg[crimg>65536] = 65536
...
#         normalize=False,
    ...
    EMGain: float
        Gain of the multiplying register
    divEMGain: boolean
        Divide image by EM gain at the end
    RN: float
        Read noise
    PCbias: float
        Bias that one can put to the readout register to read positive values.
    ...
    if nonoise:
        ...
        if lifefraction > 0.0:
            photoElectrons *= np.maximum(
                np.zeros(photoElectrons.shape),
                np.minimum(np.ones(photoElectrons.shape)
                            + lifefraction * (dqeKnee - 1.),
                            np.ones(photoElectrons.shape)
                            + lifefraction * (dqeKnee - 1.)
                            + lifefraction * dqeFluxSlope * (photoElectrons - dqeKneeFlux)))
        ...
        ######################################################################################
        if PCcorrect:
            frames *= np.exp(RN * threshold / EMGain)
            frames = -np.log(1. - frames)

        if returnFullFrame:
            if len(frames)==1: return frames[0]
            else: return frames
        else:
            if len(frames)==1: return frames[0,:yi, -xi:]
            else: return frames[:,:yi, -xi:]
