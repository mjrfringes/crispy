...
folder = '/Users/mrizzo/IFS/OS5_SIM_'+str(sampling)+'_t'+str(int(par.timeframe))+'_'+str(int(lamc))
...
averagefolder = folder+'/average_47Umac_770'
...
#                     target_star_T=5778*u.K, target_star_Vmag=4.83,     # fiducial Sun at 10 pc
                    ...
                    process_cubes=True, # this only needs to be turned to True once
...
ratio_out = SNR_spectrum(lam_midpts,signal,noise,filename=par.codeRoot+"/Inputs/Jupiter_1x_5AU_90deg.dat",albedo=0.25,lam_contrast=wavelist,outfolder=par.exportDir+"/",
...
plt.figure(figsize=(20,12))
ratio_out = SNR_spectrum(lam_midpts,final_signal,np.sqrt(final_variance),filename=par.codeRoot+"/Inputs/Jupiter_1x_5AU_90deg.dat",albedo=0.25,lam_contrast=wavelist,outfolder=par.exportDir+"/",
             FWHM=2*45./19.,FWHMdata=2,edges=1,ymargin=0.1,title='',planet_radius=1.27,planet_AU = 3.6)
